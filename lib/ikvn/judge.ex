defmodule Ikvn.Judge do
  ...
  def list_tasks(
        %Tour{} = tour,
        %Participation{role: role} = participation
      )
      when role == :admin or role == :judge do
    tour
    |> Game.list_tasks()
    |> Repo.preload(
      duties: from(d in Duty, where: d.participation_id == ^participation.id)
    )
  end
  ...
  def list_solutions(
    ...
    |> Repo.preload([
      [participation: :user],
      [marks: from(m in Mark, where: m.participation_id == ^participation.id)]
    ])
  end
