  ...
  def status(%Tournament{} = tournament) do
    cond do
      Game.tournament_is_future?(tournament) ->
        content_tag(:span, gettext("future"), class: "text-danger")
      Game.tournament_is_active?(tournament) ->
        content_tag(:span, gettext("started"), class: "text-success")
      Game.tournament_is_finished?(tournament) ->
        content_tag(:span, gettext("finished"), class: "text-secondary")
    end
  end
  ...
  def status(%Tour{} = tour) do
    cond do
      ...
      Game.tour_is_judging?(tour) ->
        content_tag(:span, gettext("judging"), class: "text-warning")
      Game.tour_is_closed?(tour) ->
        content_tag(:span, gettext("finished"), class: "text-seconary")
    end
  end
end
